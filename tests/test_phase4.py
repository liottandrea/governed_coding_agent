...
def test_build_code_authoring_agent_returns_compiled_graph() -> None:
    _load_env()
    from governed_coding_agent.subagents.code_authoring import build_code_authoring_agent
...
def test_code_authoring_interrupt_on_covers_write_and_execute() -> None:
    ...
    from governed_coding_agent.subagents import code_authoring
...
def test_system_prompt_includes_skill_md() -> None:
    _load_env()
    from governed_coding_agent.subagents.code_authoring import _build_system_prompt

    prompt = _build_system_prompt()
    assert "retrieve_knowledge" in prompt
    assert "Governed Coding Agent" in prompt
...
def test_skill_md_exists() -> None:
    skill_path = ROOT / "src" / "governed_coding_agent" / "skills" / "code-authoring" / "SKILL.md"
...
def test_retrieve_knowledge_tool_importable_from_code_authoring() -> None:
    ...
    _load_env()
    from governed_coding_agent.knowledge.retrieve import retrieve_knowledge_tool  # noqa: F401
...
def test_run_returns_expected_keys() -> None:
    ...
    with patch.object(agent, "stream", return_value=iter([{"agent": {"messages": [fake_msg]}}])):
        with patch.object(agent, "get_state", return_value=fake_state):
            from governed_coding_agent.subagents.code_authoring import run
...
def test_restricted_data_class_raises_for_frontier() -> None:
    _load_env()
    from governed_coding_agent.policy import PolicyError, check
